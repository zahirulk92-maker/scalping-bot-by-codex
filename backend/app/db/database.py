"""Versioned Database schema initialization."""
...
class Database:
    ...
    def initialize(self) -> None:
        ...
        try:
            with self.transaction() as connection:
                ...
                applied = {row[0] if isinstance(row, tuple) else row["version"] for row in connection.execute("SELECT version FROM schema_migrations")}
                migrations = {1: self._migration_1, 2: self._migration_2}
                for version, migration in migrations.items():
                    if version not in applied:
                        migration(connection)
                        if self.is_postgres:
                            connection.execute("INSERT INTO schema_migrations(version, applied_at) VALUES (%s, now())", (version,))
                        else:
                            connection.execute("INSERT INTO schema_migrations(version, applied_at) VALUES (?, datetime('now'))", (version,))
            if not self.is_postgres and self.integrity_check() != "ok":
                raise RuntimeError("SQLite integrity check failed")
            logger.info("Database initialized at %s; schema version %s", "PostgreSQL" if self.is_postgres else self.path, SCHEMA_VERSION)
    ...
    def _migration_1(self, connection: Any) -> None:
        script = """
    ...
    def _migration_2(self, connection: Any) -> None:
        script = """
