...
class PaperRepository:
    def __init__(self, database: Database) -> None:
        ...
        self.is_postgres = getattr(database, 'is_postgres', False)

    def _execute(self, connection, sql: str, params: tuple = ()): 
        if self.is_postgres:
            sql = sql.replace('?', '%s')
            sql = sql.replace('json_extract(data, \'$.is_actionable\')', 'data::json->>\'is_actionable\'')
            if 'INSERT OR IGNORE INTO paper_trades' in sql:
                sql = sql.replace('INSERT OR IGNORE', 'INSERT') + ' ON CONFLICT(trade_id) DO NOTHING'
            elif 'INSERT OR IGNORE INTO signal_snapshots' in sql:
                sql = sql.replace('INSERT OR IGNORE', 'INSERT') + ' ON CONFLICT(signal_id) DO NOTHING'
            elif 'INSERT OR IGNORE INTO risk_plans' in sql:
                sql = sql.replace('INSERT OR IGNORE', 'INSERT') + ' ON CONFLICT(plan_id) DO NOTHING'
            elif 'INSERT OR IGNORE INTO audit_events' in sql:
                sql = sql.replace('INSERT OR IGNORE', 'INSERT') + ' ON CONFLICT(event_id) DO NOTHING'
            elif 'INSERT OR REPLACE INTO equity_points' in sql:
                sql = sql.replace('INSERT OR REPLACE', 'INSERT') + ' ON CONFLICT(session_id, timestamp) DO UPDATE SET equity=excluded.equity'
            elif 'INSERT OR REPLACE INTO daily_metrics' in sql:
                sql = sql.replace('INSERT OR REPLACE', 'INSERT') + ' ON CONFLICT(session_id, metric_date) DO UPDATE SET data=excluded.data'
        return connection.execute(sql, params)

    def database_status(self) -> dict[str, object]:
        with self.database.connection() as connection:
            version = self._execute(connection, "SELECT MAX(version) FROM schema_migrations").fetchone()[0] or 0
    ...
    def prune_resolved_alerts(self, retention_days: int) -> int:
        ...
        with self.database.transaction() as connection:
            result = self._execute(connection, "DELETE FROM alerts WHERE resolved_at IS NOT NULL AND resolved_at < ?", (cutoff,))
        return int(result.rowcount)

    def resume_or_create_session(self, settings: Settings) -> tuple[PaperSession, Settings, bool]:
        with self.database.transaction() as connection:
            row = self._execute(connection, "SELECT * FROM paper_sessions WHERE status = ? ORDER BY started_at DESC LIMIT 1", (SessionStatus.ACTIVE.value,)).fetchone()
            if row:
            ...
            self._execute(connection, "INSERT INTO paper_sessions VALUES (?, ?, ?, ?, ?, ?)", (session.session_id, now.isoformat(), str(session.starting_balance), session.strategy_profile, json.dumps(session.strategy_config_snapshot, separators=(",", ":")), session.status.value))
    ...
    def load_state(self, session_id: str) -> tuple[PaperAccount | None, list[PaperPosition], list[ClosedPaperTrade]]:
        with self.database.connection() as connection:
            account_row = self._execute(connection, "SELECT data FROM paper_accounts WHERE session_id = ?", (session_id,)).fetchone()
            positions = self._execute(connection, "SELECT data FROM paper_positions WHERE session_id = ?", (session_id,)).fetchall()
            trades = self._execute(connection, "SELECT data FROM paper_trades WHERE session_id = ? ORDER BY closed_at", (session_id,)).fetchall()
    ...
    def validation_snapshots(self, session_id: str, limit: int, offset: int = 0) -> list[dict[str, object]]:
        with self.database.connection() as connection:
            rows = self._execute(connection, "SELECT data FROM validation_snapshots WHERE session_id = ? ORDER BY evaluated_at DESC LIMIT ? OFFSET ?", (session_id, limit, offset)).fetchall()
    ...
    def persist_validation_snapshot(self, session_id: str, snapshot_id: str, evaluated_at: datetime, status: str, data: dict[str, object]) -> None:
        with self.database.transaction() as connection:
            self._execute(connection, "INSERT INTO validation_snapshots(snapshot_id, session_id, evaluated_at, status, data) VALUES (?, ?, ?, ?, ?)", (snapshot_id, session_id, evaluated_at.isoformat(), status, json.dumps(data, separators=(",", ":"))))
    ...
    def persist_close(self, session_id: str, account: PaperAccount, trade: ClosedPaperTrade) -> None:
        with self.database.transaction() as connection:
            self._upsert_account(connection, session_id, account)
            self._execute(connection, "DELETE FROM paper_positions WHERE position_id = ?", (trade.position_id,))
            self._execute(connection, "INSERT OR IGNORE INTO paper_trades(trade_id, session_id, position_id, closed_at, data) VALUES (?, ?, ?, ?, ?)", (trade.trade_id, session_id, trade.position_id, trade.closed_at.isoformat(), _dump(trade)))
            self._upsert_daily(connection, session_id, account)
    ...
    def persist_signal(self, session_id: str, signal: SignalSnapshot) -> None:
        if not signal.signal_id:
            return
        with self.database.transaction() as connection:
            self._execute(connection, "INSERT OR IGNORE INTO signal_snapshots(signal_id, session_id, generated_at, data) VALUES (?, ?, ?, ?)", (signal.signal_id, session_id, (signal.generated_at or datetime.now(UTC)).isoformat(), _dump(signal)))

    def persist_plan(self, session_id: str, plan: TradePlan) -> None:
        if not plan.plan_id:
            return
        with self.database.transaction() as connection:
            self._execute(connection, "INSERT OR IGNORE INTO risk_plans(plan_id, session_id, generated_at, approved, data) VALUES (?, ?, ?, ?, ?)", (plan.plan_id, session_id, (plan.generated_at or datetime.now(UTC)).isoformat(), int(plan.approved), _dump(plan)))
    ...
    def persist_audit(self, session_id: str, event_type: str, data: dict[str, object], event_id: str | None = None) -> None:
        ...
        with self.database.transaction() as connection:
            self._execute(connection, "INSERT OR IGNORE INTO audit_events VALUES (?, ?, ?, ?, ?)", (identity, session_id, event_type, now.isoformat(), json.dumps(data, separators=(",", ":"))))

    def metrics(self, session: PaperSession) -> ForwardMetrics:
        account, _, trades = self.load_state(session.session_id)
        with self.database.connection() as connection:
            signals = self._execute(connection, "SELECT COUNT(*) FROM signal_snapshots WHERE session_id = ?", (session.session_id,)).fetchone()[0]
            actionable = self._execute(connection, "SELECT COUNT(*) FROM signal_snapshots WHERE session_id = ? AND json_extract(data, '$.is_actionable') = 1", (session.session_id,)).fetchone()[0]
            approvals = self._execute(connection, "SELECT COUNT(*) FROM risk_plans WHERE session_id = ? AND approved = 1", (session.session_id,)).fetchone()[0]
            rejections = self._execute(connection, "SELECT COUNT(*) FROM risk_plans WHERE session_id = ? AND approved = 0", (session.session_id,)).fetchone()[0]
            entries = self._execute(connection, "SELECT COUNT(*) FROM audit_events WHERE session_id = ? AND event_type = 'paper.position_opened'", (session.session_id,)).fetchone()[0]
            points = [Decimal(row[0]) for row in self._execute(connection, "SELECT equity FROM equity_points WHERE session_id = ? ORDER BY timestamp", (session.session_id,))]
    ...
    def daily_metrics(self, session_id: str, limit: int) -> list[DailyMetric]:
        with self.database.connection() as connection:
            rows = self._execute(connection, "SELECT data FROM daily_metrics WHERE session_id = ? ORDER BY metric_date DESC LIMIT ?", (session_id, limit)).fetchall()
    ...
    def equity_curve(self, session_id: str, limit: int) -> list[dict[str, str]]:
        with self.database.connection() as connection:
            rows = self._execute(connection, "SELECT timestamp, equity FROM equity_points WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?", (session_id, limit)).fetchall()
    ...
    def audit(self, session_id: str, limit: int, offset: int) -> list[dict[str, object]]:
        with self.database.connection() as connection:
            rows = self._execute(connection, "SELECT event_id, event_type, created_at, data FROM audit_events WHERE session_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", (session_id, limit, offset)).fetchall()
    ...
    def active_alerts(self, session_id: str) -> list[AlertEvent]:
        with self.database.connection() as connection:
            rows = self._execute(connection, "SELECT * FROM alerts WHERE session_id = ? AND resolved_at IS NULL ORDER BY last_seen DESC", (session_id,)).fetchall()
    ...
    def upsert_alert(self, session_id: str, code: str, severity: str, message: str, symbol: str | None, metadata: dict[str, object]) -> AlertEvent:
        now = datetime.now(UTC)
        with self.database.transaction() as connection:
            row = self._execute(connection, "SELECT * FROM alerts WHERE session_id = ? AND code = ? AND symbol IS ?", (session_id, code, symbol)).fetchone()
            if row:
                self._execute(connection, "UPDATE alerts SET last_seen = ?, alert_count = ?, severity = ?, message = ?, metadata = ?, resolved_at = NULL WHERE alert_id = ?", (now.isoformat(), row["alert_count"] + 1, severity, message, json.dumps(metadata), row["alert_id"]))
                row = self._execute(connection, "SELECT * FROM alerts WHERE alert_id = ?", (row["alert_id"],)).fetchone()
            else:
                identity = f"alert:{uuid4()}"
                self._execute(connection, "INSERT INTO alerts VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (identity, session_id, code, symbol, severity, message, now.isoformat(), now.isoformat(), 1, None, json.dumps(metadata)))
                row = self._execute(connection, "SELECT * FROM alerts WHERE alert_id = ?", (identity,)).fetchone()
        return self._alert_from_row(row)

    def resolve_alert(self, session_id: str, code: str, symbol: str | None = None) -> AlertEvent | None:
        now = datetime.now(UTC)
        with self.database.transaction() as connection:
            row = self._execute(connection, "SELECT * FROM alerts WHERE session_id = ? AND code = ? AND symbol IS ? AND resolved_at IS NULL", (session_id, code, symbol)).fetchone()
            if not row:
                return None
            self._execute(connection, "UPDATE alerts SET resolved_at = ?, last_seen = ? WHERE alert_id = ?", (now.isoformat(), now.isoformat(), row["alert_id"]))
            row = self._execute(connection, "SELECT * FROM alerts WHERE alert_id = ?", (row["alert_id"],)).fetchone()
        return self._alert_from_row(row)

    def _upsert_account(self, connection, session_id: str, account: PaperAccount) -> None:
        now = datetime.now(UTC)
        self._execute(connection, "INSERT INTO paper_accounts VALUES (?, ?, ?) ON CONFLICT(session_id) DO UPDATE SET data=excluded.data, updated_at=excluded.updated_at", (session_id, _dump(account), now.isoformat()))
        self._execute(connection, "INSERT OR REPLACE INTO equity_points VALUES (?, ?, ?)", (session_id, now.isoformat(), str(account.equity)))

    def _upsert_position(self, connection, session_id: str, position: PaperPosition) -> None:
        self._execute(connection, "INSERT INTO paper_positions VALUES (?, ?, ?, ?, ?) ON CONFLICT(position_id) DO UPDATE SET status=excluded.status, data=excluded.data, updated_at=excluded.updated_at", (position.position_id, session_id, position.status.value, _dump(position), datetime.now(UTC).isoformat()))

    def _upsert_daily(self, connection, session_id: str, account: PaperAccount) -> None:
        day = account.statistics_day
        rows = self._execute(connection, "SELECT data FROM paper_trades WHERE session_id = ?", (session_id,)).fetchall()
        ...
        self._execute(connection, "INSERT OR REPLACE INTO daily_metrics VALUES (?, ?, ?)", (session_id, day.isoformat(), _dump(metric)))
